...
**Step 3:** Use as reference when adding medications again
...
6. **NEW:** Set `isAddToMyMedication: true` to save configuration to your menu list for future reference
...
1. Always set `isAddToMyMedication: true` when adding frequently used medications
2. View menu before adding to see if medication already saved
3. Use favorites for quick access to most important medications
4. Update menu items when dosages change
5. Menu list shows what dates you used last time (helpful reference)
6. Delete old/unused items to keep menu organized
7. Search within menu to find specific medications quickly
...
### Q3: What happens if I don't specify any time slots?
...
### Q13: How do I track when I take my medications?
...
### Q21: What counts as a "missed" dose?
...
### Q27: What is the My Medication Menu List feature?
**A:** It's a personal reference list where you can save your frequently used medication configurations. When you add a medication with `isAddToMyMedication: true`, all details (dose, times, days, dates) are saved to your menu for future reference.

### Q28: How do I add a medication to my menu list?
**A:** When adding a medication via POST `/api/medications/add`, include `"isAddToMyMedication": true` in your request. The system will save the complete configuration to your menu list.

### Q29: Does the menu list auto-fill forms when I add medications?
**A:** No. The menu list is a reference/favorites list. You can view your saved configurations and manually use them as reference when filling forms. You always provide complete data when adding medications.

### Q30: What's the difference between My Medications and My Medication Menu?
**A:**
- **My Medications** (`/my-medications`): Shows your current active medication schedules
- **My Medication Menu** (`/menu`): Shows your saved medication configurations for reference, including medications you've used before

### Q31: Can I have duplicate medications in my menu list?
**A:** The system prevents exact duplicates based on medication ID. If you try to add the same medication to menu again, it returns the existing menu item instead of creating a duplicate.

### Q32: Will deleting a menu item delete my medication schedule?
**A:** No. Deleting from menu list only removes the reference. Your active schedules are not affected. Menu and schedules are separate.

### Q33: What are favorites in the menu list?
**A:** Favorites let you mark important medications for quick access. Use POST `/menu/{uuid}/toggle-favorite` to mark/unmark, then GET `/menu/favorites` to view only your favorites.

### Q34: Why does my menu item show dates?
**A:** The menu saves the complete configuration including the dates you used last time. This serves as a helpful reference when you add the medication again - you can see what date range you used before.

### Q35: Can I update a menu item?
**A:** Yes! Use PUT `/menu/{uuid}` to update any field in a menu item (dose, times, days, dates, notes, etc.). This is useful when your medication dosage or timing changes.
...
**Changelog:**
- v2.3 (Oct 16, 2025): Added My Medication Menu List feature with 6 new endpoints (`/menu`, `/menu/favorites`, `/menu/{uuid}`, `/menu/search`, `/menu/{uuid}` PUT/DELETE, `/menu/{uuid}/toggle-favorite`); Added `isAddToMyMedication` field to `/add` endpoint; Enables users to save and manage frequently used medication configurations; Implements secure UUID-only responses (no internal IDs exposed)
